...
模型选择列表里最新的那个（本文演示时用的是 GPT-5.5，现在最新的是 GPT-6 Astra），速度选「标准」就够了，智能程度选「高」。
...
而且对我来说比较难受的是有效上下文偏小，GPT-5.5 在 Codex APP 的订阅模式下只有 258K，做大型项目重构时跟其他 AI 编程工具和模型支持的 1M 上下文比还是有差距。不过这一点在 GPT-6 Astra 上已经明显改善了，它的上下文窗口做到了 105 万 token，但要注意单次请求输入超过 27.2 万 token 之后 API 会跳到更贵的长上下文费率。
