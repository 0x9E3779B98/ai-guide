...
- DeepSeek V4.1 Flash：5520 亿参数（采用全新模型结构的 MoE 混合专家架构，原生多模态）
- DeepSeek-V4-Pro：1.6 万亿参数（采用 MoE 混合专家架构，实际激活 490 亿，已于 2026 年 9 月下线）
- Qwen3.8-Max：2.4 万亿参数（通义千问旗舰模型，同样是 MoE 架构，实际激活 950 亿，只占总参数的 4% 左右）
- Kimi K3：2.8 万亿参数（月之暗面旗舰模型）
- GLM-5.3：7530 亿参数（智谱旗舰模型，MoE 架构）
- GLM-5.3-Flash：3200 亿参数（激活仅 180 亿，智谱首个原生多模态模型）
- Llama 4 Scout：1090 亿参数（Meta 开源模型，激活 170 亿）

这里正好能看出一个有意思的趋势：**参数量和能力已经不是简单的正相关了**。DeepSeek V4.1 Flash 只有 5520 亿参数，却在各项指标上全面超越了 1.6 万亿参数的 V4 Pro；GLM-5.3-Flash 激活参数只有 180 亿，综合智能指数却能跟顶级旗舰打平。模型结构的创新，比单纯堆参数更有效。
